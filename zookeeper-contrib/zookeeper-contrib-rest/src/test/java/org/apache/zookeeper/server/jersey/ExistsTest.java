...
public class ExistsTest extends Base {
    ...
    private void verify(String path, ClientResponse.Status expectedStatus,
            String type) {
    ...
    @ParameterizedTest
    @MethodSource("data")
    public void testExists(String path, ClientResponse.Status expectedStatus)
            throws Exception {
        verify(path, expectedStatus, MediaType.APPLICATION_OCTET_STREAM);
        verify(path, expectedStatus, MediaType.APPLICATION_JSON);
        verify(path, expectedStatus, MediaType.APPLICATION_XML);
    }
}
